...
| `init` | 生成沙箱 Dockerfile 模板，或初始化自托管沙箱项目。 |
| `config` | 读取、写入或删除沙箱命令默认值。 |
| `create` | 创建沙箱工具。 |
| `delete` | 删除沙箱工具或指定会话。 |
| `dashboard` (`ui`) | 启动本地沙箱 Web 控制台。 |
| `list` | 列出本地或云端沙箱会话，也可列出沙箱 Tool。 |
...
生成沙箱 Dockerfile 模板。未指定模板时，默认生成 `skill` 模板；传入 `self-host` 时会同时生成 `Dockerfile`、`.agentkit/sandbox.yaml`、`.gitignore`、`.dockerignore` 和 `README.md`，用于自托管沙箱环境。
...
| `self-host-project-name` | string | `default` | 自托管 Tool 与 Runtime 使用的 AgentKit 项目。 |
| `self-host-poll-interval-seconds` | integer | `5` | 自托管 Tool 与 Runtime 就绪检查的轮询间隔。 |
| `self-host-environment-base-url` | string | — | 自托管环境网关地址。 |
| `self-host-environment-id` | string | — | 自托管环境 ID。 |
| `self-host-environment-key` | string | — | 自托管环境密钥；列表输出会隐藏该值。 |
| `self-host-sandbox-id` | string | `sandbox create` 写入 | 自托管 Runtime 绑定的沙箱 Tool ID。 |
| `self-host-sandbox-name` | string | `sandbox create` 写入 | 自托管 Runtime 绑定的沙箱 Tool 名称。 |
| `self-host-runtime-image-url` | string | 内置运行时镜像 | 自托管 Runtime 镜像地址。 |
| `self-host-runtime-name` | string | `agentkit-selfhost-runtime` | 自托管 Runtime 名称前缀。 |
| `self-host-runtime-role-name` | string | `AgentKit_Runtime_Default_ServiceRole` | 自托管 Runtime 使用的 IAM 角色。 |
| `self-host-runtime-cpu-milli` | integer | `2000` | 自托管 Runtime CPU，单位为毫核。 |
| `self-host-runtime-memory-mb` | integer | `4096` | 自托管 Runtime 内存，单位为 MB。 |
| `self-host-runtime-min-instance` | integer | `1` | 自托管 Runtime 最小实例数。 |
| `self-host-runtime-max-instance` | integer | `1` | 自托管 Runtime 最大实例数。 |
| `self-host-runtime-max-concurrency` | integer | `10` | 自托管 Runtime 单实例最大并发数。 |
| `self-host-runtime-timeout-seconds` | integer | `1200` | 自托管 Runtime 请求超时时间。 |
| `self-host-runtime-api-key-name` | string | `Authorization` | 自托管 Runtime API Key 请求头名称。 |
...
| `--llm-shield-app-id <app-id>` | 为 `Skill` 工具启用 LLM Shield，并注入指定 App ID。 | — |
| `--envs <KEY=VALUE>` | 注入到沙箱工具的环境变量；可重复。重复键会报错。 | — |
| `--json` | 输出 JSON。 | `false` |
...
为 `Skill` 工具启用 LLM Shield 或注入自定义环境变量时，可在创建工具时传入对应标志：
...
自托管项目使用 `sandbox init -t self-host` 生成配置，再通过 `sandbox build`、`sandbox create` 和 [`env create`](/productions/agentkit-cli/preview/zh/commands/env) 创建完整环境。`sandbox create` 会读取 `project_type: self-host`，使用自托管工具命名规则，并把创建出的 Tool ID 写入 `self_host.sandbox.id`。
...
列出沙箱会话或沙箱 Tool，并输出 JSON。默认读取本地缓存；传入 `--remote` 时从云端查询，列出远端会话时必须提供 `--tool-id`。

| 标志/参数 | 说明 | 默认值 |
| --- | --- | --- |
| `-s, --session-id <id>, --sid <id>` | 要查找的用户会话 ID。 | — |
| `--tool-id <id>` | 按沙箱 Tool ID 过滤；远端会话列表必填。 | — |
| `--sessions` | 列出沙箱会话。 | 默认模式 |
| `--tools` | 列出沙箱 Tool。 | `false` |
| `--tool-name <name>` | `--tools` 模式下按 Tool 名称过滤。 | — |
| `--tool-type <type>` | `--tools` 模式下按 Tool 类型过滤。 | — |
| `--status <status>` | `--tools` 模式下按状态过滤。 | — |
| `--local` | 仅查询本地缓存。 | 默认 |
| `--remote` | 仅查询云端数据。 | `false` |
...
打开沙箱 Web 预览，并输出包含 URL、工具 ID、会话 ID 与会话是否新建的 JSON。默认会先在沙箱浏览器中打开 `/home/gem/` 文件路径，再打开本机 noVNC 页面。
