...
通过浏览器 SSO 完成鉴权，并存储短时 STS 凭据。传入 `--identity-only` 时，CLI 只保存用户 OIDC 会话，不创建或刷新 STS 凭据；该模式用于调用 `custom_jwt` 运行时或 [`chat`](/productions/agentkit-cli/preview/zh/commands/chat) 共享 Harness。
...
| `--identity-only` | 只保存用户 OIDC 会话，不创建 AgentKit 管理凭据。 | `false` |
...
| `--identity-only` | 保存不含 STS 角色的 OIDC-only profile。 | `false` |
...
为已有用户池创建或复用 CLI 登录资源，并把 `/.well-known/agentkit-cli` 发现文档发布到指定 TOS bucket。也可以同时传入已有 `--issuer`、`--client-id`、`--role-trn` 与 `--provider-trn` 进入仅发布模式，避免重新创建身份和 IAM 资源。
...
| `--account <account>` | 预期的账号 ID；当前凭证不属于该账号时拒绝继续 | — |
| `--region <region>` | 云区域 | 当前云厂商的默认区域 |
| `--domain <domain>` | 已配置 CNAME 与 HTTPS 证书的自定义登录域名。 | TOS bucket 的 HTTPS 地址 |
| `--issuer <url>` | 已有 OIDC issuer；与 `--client-id`、`--role-trn`、`--provider-trn` 同时传入时启用仅发布模式。 | — |
| `--client-id <id>` | 已有公开 OIDC client id；仅发布模式必填。 | — |
| `--role-trn <trn>` | 目标账号下已有 STS role TRN；仅发布模式必填。 | — |
| `--provider-trn <trn>` | 目标账号下已有 IAM OIDC provider TRN；仅发布模式必填。 | — |
