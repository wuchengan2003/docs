...
Authenticate via browser SSO and store short-lived STS credentials. With `--identity-only`, the CLI only stores the user's OIDC session and does not create or refresh STS credentials; use this mode to invoke `custom_jwt` runtimes or [`chat`](/productions/agentkit-cli/preview/en/commands/chat) with a shared Harness.
...
| `--identity-only` | Store only the user OIDC session, without AgentKit management credentials. | `false` |
...
```bash
agentkit auth profile set my-profile --issuer https://example.com --client-id abc123 --identity-only
```
...
Create or reuse the CLI login resources for an existing user pool and publish the `/.well-known/agentkit-cli` discovery document to a selected TOS bucket. You can also pass existing `--issuer`, `--client-id`, `--role-trn`, and `--provider-trn` together to use publish-only mode without creating identity or IAM resources.
...
| `--account <account>` | Expected account ID; refuse to continue when the active credentials belong to another account | — |
| `--region <region>` | Cloud region | Current provider's default region |
| `--domain <domain>` | Custom login hostname with CNAME and HTTPS already configured. | TOS bucket HTTPS address |
| `--issuer <url>` | Existing OIDC issuer; enables publish-only mode when passed together with `--client-id`, `--role-trn`, and `--provider-trn`. | — |
| `--client-id <id>` | Existing public OIDC client id; required in publish-only mode. | — |
| `--role-trn <trn>` | Existing target-account STS role TRN; required in publish-only mode. | — |
| `--provider-trn <trn>` | Existing target-account IAM OIDC provider TRN; required in publish-only mode. | — |
