...
- [New command] Added the [`skills secure`](/productions/agentkit-cli/archives/0.52.8/en/commands/skill#skills-secure) command for bulk generation of secure Skill wrappers from a local skills directory. The generated wrappers keep `name` and `description` but do not copy the real Skill prompt, references, scripts, dependencies, or credentials.
...
- [Sandbox commands] [`sandbox config`](/productions/agentkit-cli/archives/0.52.8/en/commands/sandbox#sandbox-config) adds self-host config keys and redacts `self-host-environment-key` in `--list` output, while moving non-self-host `tool-id` / `tool-name` config into `tool_info` so Tool identifiers no longer live under session config.

## 0.52.7 - 2026-08-25

- [New command] Added the [`env`](/productions/agentkit-cli/archives/0.52.8/en/commands/env) command group. After a Claude self-host sandbox project completes `sandbox build`, `env create` can create the Private Tool and dispatcher Runtime from the built image and `.env`, and `env status` refreshes local environment state.
...
- [init commands] [`init --from-agent`](/productions/agentkit-cli/archives/0.52.8/en/commands/init) supports the `agent_server` wrapper type, and `--template basic|basic_stream|agent_server` can select a compatible wrapper template; mismatched `--template` and `--wrapper-type` values fail early.
...
- [runtime commands] [`runtime show --json`](/productions/agentkit-cli/archives/0.52.8/en/commands/runtime#runtime-show) now returns `toolId` when a Runtime is bound to a Tool, making Runtime-to-Tool association available to scripts.
...
- [Configuration] [`config --init`](/productions/agentkit-cli/archives/0.52.8/en/commands/config#config) now writes project-level options from the same command into the newly created `agentkit.yaml`, making complete lifecycle config generation usable in non-interactive environments.
...
- [Sandbox commands] [`sandbox codex-login`](/productions/agentkit-cli/archives/0.52.8/en/commands/sandbox#sandbox-codex-login) now writes Codex OAuth config directly inside the sandbox session before injecting local subscription credentials, so subscription login setup no longer depends on passing generated config through session environment variables.
...
- [New command] Added the [`chat`](/productions/agentkit-cli/archives/0.52.8/en/commands/chat) command for OAuth-bound conversations with shared Harnesses published through tenant login discovery aliases, with sessions isolated by verified user identity.
- [eval commands] [eval backend](/productions/agentkit-cli/archives/0.52.8/en/commands/eval/backend) now resolve Coze or TEA through the AgentKit evaluation gateway by default and add `eval backend`, `--cache-refresh`, and `--verbose` for inspecting backend coordinates and troubleshooting TEA requests.
...
- [Sandbox commands] [`sandbox create`](/productions/agentkit-cli/archives/0.52.8/en/commands/sandbox#sandbox-create) adds `--llm-shield-app-id` for enabling LLM Shield on `Skill` sandbox tools.
...
- [Harness Sidecar] [Release configuration](/productions/agentkit-cli/archives/0.52.8/en/agentkit-yaml#harness-sidecar) now supports managed `harness_sidecar` config, adds runtime descriptions, create-time tags, and Code Pipeline workspace / pipeline pinning fields, and derives `Auto` TOS buckets from the cloud provider, region, and account.
- [Harness Sidecar] [`release`](/productions/agentkit-cli/archives/0.52.8/en/commands/release#release) adds `--json` for managed Harness Sidecar releases, emitting newline-delimited build, deploy, runtime-id, and final-result events and verifying the Sidecar, model proxy, and required MCP gateway after release.
...
- [Configuration] [Config commands](/productions/agentkit-cli/archives/0.52.8/en/commands/config) now validate region values, accepting only lowercase, hyphen-separated region codes so invalid regions cannot enter service endpoints or request signing.

## 0.51.2 - 2026-08-11

- [New command] Added the [Model Gateway commands](/productions/agentkit-cli/archives/0.52.8/en/commands/model-gateway) for activating a gateway, managing model providers and consumers, and generating `curl`, OpenAI, Anthropic, or AgentKit calling examples from authorized models.
...
- [New command] Added the [`harness invoke`](/productions/agentkit-cli/archives/0.52.8/en/commands/harness#harness-invoke) command for directly invoking a deployed Harness with per-request overrides for the model, system prompt, tools, skills, runtime backend, A2A registry, and maximum LLM calls, with either `invoke` or `run_sse` transport.
- [Auth commands] Added [identity-only mode](/productions/agentkit-cli/archives/0.52.8/en/commands/auth): `login --identity-only` and `auth profile set --identity-only` can store only the OIDC session. For matching `custom_jwt` Harness invocations, the CLI can forward that user identity without creating management credentials.
