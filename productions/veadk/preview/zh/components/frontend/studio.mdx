...
Dev Sandbox 技能生成需要管理员配置处于 `Ready` 状态的 AgentKit DevEnv Tool。本地启动时通过 `SANDBOX_DEV` 环境变量指定 Tool ID；部署到 VeFaaS 时通过 `--sandbox-dev-tool-id` 自动创建或复用。该 Tool 仅支持火山引擎部署。未配置时，技能空间的管理和上传功能不受影响，但「创建技能」和「优化」入口会显示「管理员未配置 Dev Sandbox」并禁用。
...
| 配置项 | 说明 |
| :--- | :--- |
| 目标 | 用自然语言描述希望技能完成的任务，至多 20000 字符。 |
| Skill 名称 | 可选。只能包含小写字母、数字和连字符，至多 64 字符；留空时自动生成。 |
| 生成配置 | 每组配置启动独立的 Dev Sandbox 会话。最多可添加 3 组，每组选择模型和风格后并行生成候选方案。 |
...
未指定 `--sandbox-chat-codex-tool-id` 时，部署命令会在 `--region` 指定的地域创建内置智能体使用的 AgentKit CodeEnv Tool；火山引擎部署还会额外创建一个 DevEnv Tool 用于开发沙箱。这些 Tool 创建的 Session 与 VeFaaS Function、API Gateway 保持同一地域。模型凭据只配置在各自 Tool 中，VeFaaS Function 只接收 Tool ID。若已有符合要求且地域一致的 Tool，可通过参数直接复用。
...
登录后，侧边栏底部的账号菜单提供「系统信息」入口，选择后进入系统信息页面，展示 Studio 的版本、沙箱信息和用户池。该页面仅对具有智能体管理权限的用户开放，所有资源标识均为只读。
