...
配置 LLM 智能体时，可选择填写模型 API 地址。Studio 根据该地址是否为当前云服务商的官方 Ark 端点，采用不同的凭据策略：

| 模型 API 地址 | Studio 提供的凭据 | 调试运行 | 部署 |
| :--- | :--- | :--- | :--- |
| 留空 | 使用官方 Ark 地址与 Studio 管理的 Ark API Key | 支持 | 无需额外填写 |
| 当前云的官方 Ark 地址 | 使用该地址与 Studio 管理的 Ark API Key | 支持 | 无需额外填写 |
| 自定义地址（非官方 Ark 端点） | 不提供凭据 | 不支持 | 需在发布页填写该智能体的 API Key |

<Warning>
  调试运行不支持自定义模型地址。使用非官方 Ark 端点的智能体在调试时会报错，需改用当前云的官方 Ark 地址，或先部署再通过 Runtime 测试。
</Warning>

使用自定义模型地址时，发布页会在环境变量区域显示「自定义模型凭据」分组，为每个使用自定义地址的智能体提供必填的 API Key 输入框。输入的凭据仅用于本次发布，不保存在草稿中。生成的项目代码通过 `CUSTOM_MODEL_<智能体名称>_API_KEY` 环境变量读取该凭据，并在 `.env.example` 中以占位符形式列出。
