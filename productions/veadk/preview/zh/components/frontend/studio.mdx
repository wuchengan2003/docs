...
产物标签页集中展示对话中生成的文档、图片和视频等产物。产物按会话来源分组，显示所属应用、会话和生成时间，支持按类型筛选和搜索。点击产物可预览图片或视频，文档类产物支持在线预览。

<Note>
  产物从已部署智能体的会话事件中收集。如需查看产物，确保 Studio 已连接到已部署的 Runtime 并存在包含产物事件的会话。
</Note>
