...
- `region`：未显式设置时读取环境变量 `DATABASE_VIKING_REGION`；仍为空时，`byteplus` 默认为 `ap-southeast-1`，其余默认为 `cn-beijing`。
...
| `DATABASE_VIKING_REGION` | `cn-beijing`（byteplus 为 `ap-southeast-1`） | 记忆库区域。 |
