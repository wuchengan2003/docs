...
The new-conversation view provides two modes visible to all users:
...
Dev Sandbox skill generation requires an administrator to configure an AgentKit DevEnv Tool in the `Ready` state. For local startup, specify the Tool ID via the `SANDBOX_DEV` environment variable; for VeFaaS deployments, it is auto-created or reused via `--sandbox-dev-tool-id`. This Tool is supported only for Volcengine deployments. When not configured, skill space management and upload remain available, but the "Create skill" and "Optimize" actions show "Dev Sandbox not configured" and are disabled.
...
| Setting | Description |
| :--- | :--- |
| Goal | Describe what you want the skill to accomplish in natural language, up to 20,000 characters. |
...
| Generation groups | Each group starts an independent Dev Sandbox session. Up to 3 groups can be added; each selects a model and style to generate candidate skills in parallel. |
...
When `--sandbox-chat-codex-tool-id` is omitted, deployment creates the AgentKit CodeEnv Tool for built-in agents in the region selected by `--region`; Volcengine deployments also create an additional DevEnv Tool for the Dev Sandbox. Sessions created by these Tools use the same region as the VeFaaS Function and API Gateway. Model credentials are configured only on the respective Tools; the VeFaaS Function receives only the Tool IDs. You can pass existing Tool IDs when suitable Tools are already available in the same region.
...
After signing in, the account menu at the bottom of the sidebar offers a "System info" entry. Selecting it opens the system information page, which displays the Studio version, sandbox information, and user pools. This page is available only to users with agent-management access, and all resource identifiers are read-only.
