...
4. In the Environment step, select command-line tools to bake into the cloud Runtime image, or customize the Dockerfile used to build the image; this step is optional.
5. Select AgentKit deployment and monitor the build-image, deploy, and publish stages from the workspace. After deployment, the agent appears as published in the workspace and can be updated on the same Runtime.
...
<Steps>
  ...
  <Step title="Build and validate">
    Studio creates an intelligent-development sandbox session where Codex outlines the goal and implementation, then writes, runs, and validates the agent code. Progress and intermediate results are visible in the conversation in real time. The development environment is retained for up to 8 hours and can be iterated on within the same session.
  </Step>
