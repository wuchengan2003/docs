...
Studio capabilities such as video creation and automatic evaluation require persistent object storage to save reference assets, generated results, and optimization snapshots. Studio uses TOS as the persistent storage backend. When no dedicated publish storage is configured, skill publishing also reuses the persistent storage bucket; see [Skill publish storage](#skill-publish-storage).
...
Use `--site-title` to set a system name of up to 16 characters and `--site-logo` to provide a local image or HTTP(S) image URL. The logo appears in the sidebar, login page, and browser favicon, while the system name becomes the browser title. Omitting `--site-title` uses the default `AgentKit Studio` name.
...
### Model credentials and custom model endpoints

When configuring an LLM agent, you can optionally provide a model API base URL. Studio applies different credential strategies depending on whether the URL is the official Ark endpoint for the current cloud provider:

| Model API base URL | Credential provided by Studio | Debug run | Deployment |
| :--- | :--- | :--- | :--- |
| Blank | Official Ark endpoint with a Studio-managed Ark API Key | Supported | No additional input required |
| Official Ark endpoint for the current cloud | That endpoint with a Studio-managed Ark API Key | Supported | No additional input required |
| Custom endpoint (non-official Ark) | No credential | Not supported | You must provide the agent's API Key on the publish page |
...
When using a custom model endpoint, the publish page displays a "Custom model credentials" group in the environment-variables area, providing a required API Key input for each agent that uses a custom endpoint. The credential is used only for that publish and is not saved in the draft. Generated project code reads the credential from the `CUSTOM_MODEL_<agent_name>_API_KEY` environment variable, and `.env.example` lists it as a placeholder.
