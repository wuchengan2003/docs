...
When deploying Studio, APMPlus frontend telemetry can collect Studio instance visits, sign-in usage, agent deployment outcomes, and sandbox creation outcomes. Telemetry is disabled by default and is enabled only when both the APMPlus Client aid and token are configured; if it is unconfigured or reporting fails, Studio continues to work normally.
...
<Note>
  Telemetry collects only usage dimensions such as deploy ID, user ID, role, region, source, and deployment phase outcomes. It does not collect conversation content, prompts, generated code, environment variable values, or any secrets. In-app updates of Studio preserve the configured APMPlus environment variables.
</Note>
