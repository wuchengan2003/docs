...
- `region`: when not set explicitly, reads env `DATABASE_VIKING_REGION`; when still empty, `byteplus` defaults to `ap-southeast-1` and others default to `cn-beijing`.
...
| `DATABASE_VIKING_REGION` | `cn-beijing` (`ap-southeast-1` for byteplus) | Memory region. |
